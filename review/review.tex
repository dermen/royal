...
Kam showed that if the distribution of particle rotations dictated by $\hat{R}_\omega$ is isotropic, distributed uniformly over $\omega$, then the structure factor correlation function
...
\ee
where $n_s(\bm q)$ is the total photons scattered from all particles in snapshot $s$ into a pixel along scattering vector $\bm q$. Neglecting inter-particle scattering interference, $n_s(\bm q)$ is governed by a Poisson distribution $\mathscr{P}(n_s(\bm q) \,|\, \mathscr{N}_s(\bm q))$ with mean
\be \label{photon}
\mathscr{N}_s(\bm q) \propto \sum_{\omega_\alpha \in \,\Gamma_{s} }^N S(\bm q,\omega_\alpha)
 \ee
