...
\]
 ...
 and $\mu_s( |\vec{q}_{hkl}|)$ is the average photon counts per Bragg ring per snapshot. Assuming proper sampling of orientation space per snapshot, and sufficiently large numbers of snapshots, it can be argued that $D (|\vec{q}_{hkl}|,|\vec{q}_{h'k'l'}|, \Delta  )$ and the Kam correlation $C (|\vec{q}_{hkl}|,|\vec{q}_{h'k'l'}|, \psi  )$ contain the same information content. Such discussions, however, are beyond the scope of this report. One should expect CXS signal at values of $\Delta $ corresponding to the geometry of the reciprocal lattice and the angles between reciprocal lattice vectors. For instance, $D (|\vec{q}_{111}|,|\vec{q}_{111}|, \Delta  )$ should display strong signal at $\Delta_1 = \arccos[ \frac{-2}{3\cos^{2}\theta} + 1  ]$ and $\Delta_2 = \arccos[ \frac{-4}{3\cos^{2}\theta} + 1  ]$ where $2\theta$ is the standard scattering angle.
