...
\label{fig:binary}
\begin{center}
\includegraphics[ ]{../figures/binary.png}
\end{center}
{\bf Figure 1} {\bf A} Typical measurement of $n_{s}( q_{111},\phi)$, i.e. the photon counts around the Bragg ring at $q_{111}$. The horizontal bars show the binary cutoff for each module containing the ring. {\bf B} histogram of the intensities in {\bf A} on a log scale. Notice the tail at higher intensities, indicating the presence of large particles. {\bf C} Result of applying the binary filter to {\bf A}. A binary shot at $q_{111}$ is, on average, 10\% ones, 77\% zeros and 13\% masked pixels ( masked pixels are ignored in the analysis )  {\bf D} The average binary intensity from a scan of 500 snapshots. Notice how some of the regions are biased, indicating in intra-module pixel variations. {\bf E} Typical snapshot. Low/high intensities shown in white/black. The Bragg rings at $q_{111}$ and $q_{200}$ are clearly visible.
\end{figure}


Data from 15,496 x-ray diffraction images of silver NP solution were collected and analyzed at the micro-crystallography beamline (12-2) at SSRL. We prepared a sample containing an estimated $10^{9}$  20 nm NPs per snapshot in the illuminated volume, but we observed significant numbers of NPs that were larger. Samples were loaded and oriented in the X-ray beam using the Stanford Automated Mounting System (SAM), controllable from the experimental hutch. Using a liquid nitrogen-cooled double crystal monochromator we tuned the beam energy to 17 keV. The beam was focused down to about $20 \times 50 \mu$m$^2$ using Rh coated Kirkpatrick-Baez mirrors. Snapshots were recorded on a Dectris Pilatus 6M photon counting detector. 
